...
impl LsmStorageInner {
    // 需要引入same_as_last_key来保证同一个key的不同版本在同一个sstable里面，既然在同一个sstable里面的话，还需要判断是否是bottom level吗？
    fn compact_generate_sst_from_iter(
        ...
        // 此处直接拿到一个watermark，不需要包证在compact的过程中，watermark不变的吗？不需要，因为后续开启的事务的timestamp肯定
        // 更大，只要保证现在这个时刻拿到的timestamp是安全compact的那么后续也不会出问题。
        ...
        'outer: while iter.is_valid() {
            ...
            // 只有当当前key不等于之前的key的时候，same_as_last_key才为false，first_key_below_watermark才为true。
            // 这个first_key_below_watermark只是用来标识是不是这个key的最大的那个版本。
            if !same_as_last_key {
            ...
            if iter.key().ts() <= watermark {
                ...
                if !first_key_below_watermark {
                    // 这里就进行了删除，first_key_below_watermark为false表示不是第一个小于watermark的版本。
                    iter.next()?;
                    continue;
                }
