...
impl BlockIterator {
    ...
    fn seek_to_offset(&mut self, offset: usize) {
        ...
        self.key.append(key);
        // 由于key_len并不是通过get_xxx读出来的，所以需要调用advance手动更改position
        ...
        // offset是这个key/value开始的位置，这里只需要value的开始和结束位置，所以value的开始位置需要通过offset加上
        // 一个overlap_len（u16） + key_len（u16） + key（key真实长度） + ts（u64） + value_len（u16）
        let value_offset_begin =
            offset + SIZEOF_U16 + SIZEOF_U16 + std::mem::size_of::<u64>() + key_len + SIZEOF_U16;
        // 结束位置就是value_offset_begin + value_len（value占用了多少空间）
