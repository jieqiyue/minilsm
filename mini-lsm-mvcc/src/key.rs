...
// KeySlice<'a> = Key<&'a [u8]>
// 承载 user_key：引用切片 &[u8]
// 不拥有数据，只借用一段内存（受生命周期 'a 约束）
// 几乎零开销（只是指针+长度+ts）
// 适合：读路径/比较/seek/迭代器（不想拷贝 key）
pub type KeySlice<'a> = Key<&'a [u8]>;
// KeyVec = Key<Vec<u8>>
// 承载 user_key：Vec<u8>
// 拥有数据（堆分配，可增长/可修改）
// clone 会复制内容（相对贵）
// 适合：写路径、构造 key、需要可变拼接/复用 buffer 的地方
pub type KeyVec = Key<Vec<u8>>;
// KeyBytes = Key<Bytes>
// 承载 user_key：Bytes（引用计数/共享 buffer）
// 拥有数据，但 clone 很便宜（通常只是增加引用计数，不拷贝内容）
// 更适合不可变数据共享
// 适合：SST/Block 里保存的 key、memtable/iter 之间共享 key、减少拷贝
...
// 原先是只比较0，也就是比较key，key更小的也就是更小，现在要比较时间戳了，并且时间戳的更大的，反而返回更小，也就是更新。
// ("a", 233) < ("a", 0) < ("b", 233) < ("b", 0)
// 这个比较有什么用呢？