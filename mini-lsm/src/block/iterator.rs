...
impl BlockIterator {
    ...
    pub fn seek_to_key(&mut self, key: KeySlice) {
        ...
        // search key less than first key in the block
        // low  = 0   high  = 3   mid =  1
        // low = 0  high = 1     mid = 0
        // low = 0 high = 0
        // return 0

        // search key greater than last key in the blcok
        // low  = 0   high  = 3  mid = 1
        // low = 2    high = 3  mid = 2
        // low = 3    high =  3
        // return 3
        while low < high {
            ...
            match self.key().cmp(&key) {
                // 有一种情况是这个key比所有的key都小，那么上层会拿到一个idx为0的下标，那么最终返回的low会是0，也就是这个block的第一个
