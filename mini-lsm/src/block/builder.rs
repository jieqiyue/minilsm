...
pub struct BlockBuilder {
    /// Offsets of each key-value entries.  每个元素的偏移量
    offsets: Vec<u16>,
    /// All serialized key-value pairs in the block. 每个元素的信息，以及key和value的信息都存放在这里
    data: Vec<u8>,
    /// The expected block size.
    block_size: usize,
    /// The first key in the block  存放这个block的第一个key，用来减少后续key的存放，因为在put一个key/value的时候，会计算和这个first_key的overlap的大小
    first_key: KeyVec,
}
