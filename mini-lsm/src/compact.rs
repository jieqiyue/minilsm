...
impl LsmStorageInner {
    // tiered和Leveled这两种方式都会方式都会使用这个方法生成sst
    // 但是这个方法生成的sst id它们应该被放到什么地方呢？由于底层是抽取了一些sst id出来进行合并的，
    // 这里面就是使用SsTableBuilder来生成sst
    fn compact_generate_sst_from_iter(
        ...
        while iter.is_valid() {
            ...
            if compact_to_bottom_level {
                if !iter.value().is_empty() {
                    // 由于iter这个迭代器是能按有序的方式拿到key的，所以这个add里面可以判断first key是什么
        ...
        // 如果iter中的所有的信息都append完了，但是又不是刚好达到最后一个SsTableBuilder的阈值的话，那么就还有残留
    ...
    fn compact(&self, task: &CompactionTask) -> Result<Vec<Arc<SsTable>>> {
        ...
        match task {
            ...
            | CompactionTask::Leveled(LeveledCompactionTask {
                ...
                // 如果是l0的合并的话，upper_level是none
                None => {
                    // 由于l0是无序的，所以要用SsTableIterator
                    ...
                    // 由于除了l0都是有序的，所以拿到底层被合并的那层的有重叠的sst id，并且构造一个SstConcatIterator
                    let mut lower_ssts = Vec::with_capacity(lower_level_sst_ids.len());
                    for id in lower_level_sst_ids.iter() {
                        lower_ssts.push(snapshot.sstables.get(id).unwrap().clone());
                    }
    ...
    fn trigger_compaction(&self) -> Result<()> {
        ...
        // 这里生成的sstables都是新的，所以这里得到的sst_id也是新的
