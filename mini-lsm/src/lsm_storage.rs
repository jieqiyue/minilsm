...
impl LsmStorageInner {
    ...
    pub(crate) fn open(path: impl AsRef<Path>, options: LsmStorageOptions) -> Result<Self> {
        ...
        let abs_path: PathBuf = fs::canonicalize(path)?; // 这里得到绝对路径
        ...
        if !manifest_path.exists() {
            ...
            for record in records {
                match record {
                    ...
                    // apply_compaction_result都是针对status里面的sst id进行操作
            ...
            if options.enable_wal {
                let mut wal_cnt = 0;
                // 到这里还在memtables中的就是剩下的memtable，因为上面按照顺序重放的时候将一些已经flush的已经移除掉了的
